...
namespace fCraft {
    static class MapCommands {
        ...
        internal static void Init() {
            ...
            Commands.AddCommand("logic", SwitchLogic, false);
            Commands.AddCommand("logic3d", SwitchLogic3d, false);
            Commands.AddCommand("physics", S_Physics, false);
            Commands.AddCommand("phyadd", Phyadd, false);
            Commands.AddCommand("phyrem", Phyrem, false);
            Commands.AddCommand("phychk", Phycheck, false);
            Commands.AddCommand("phymode", Phymode, false);
            Commands.AddCommand("wamode", CModeWater, false);
        ...
        internal static void Phymode(Player player, Command cmd)
        {
            if (!player.Can(Permissions.ControlPhysics))
            {
                player.NoAccessMessage(Permissions.ControlPhysics);
            ...
            try { BlockMode = Convert.ToByte(blockpar2); }
            catch
            {
        ...
        internal static void Phycheck(Player player, Command cmd)
        {
            string blockpar = cmd.Next();
            int BlockAddr = -1;
            try { BlockAddr = Convert.ToInt32(blockpar); }
            ...
            {
                player.Message("Block '" + blockpar + "' is not hpysicsized.");
            }
        }
